...
//	private static final int dimension = 3; 	// 拼图的维数
	...
	public int hashCode() {
	    ...
	    // BKDRHash
	    // O(N^2)
	    for (int i = 1; i < this.nodesState.length; ++i) {
	        hash = hash * seed + this.nodesState[i];
	    }
	    return hash;
	}
}

final class OrderedJigsawNode {
    static int lastTimestamp = 0;
    public JigsawNode node;
    public int timestamp;
    public OrderedJigsawNode(JigsawNode node) {
        this.node = node;
        timestamp = lastTimestamp++;
    }
}

final class JigsawNodeComparator implements Comparator<OrderedJigsawNode> {
    @Override
    public int compare(OrderedJigsawNode a, OrderedJigsawNode b) {
        if (a.node.getEstimatedValue() != b.node.getEstimatedValue())
            return a.node.getEstimatedValue() - b.node.getEstimatedValue();
        else
